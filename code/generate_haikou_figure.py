...
# -*- coding: utf-8 -*-
"""
Generate Spatial Distribution and Grid Partition Visualization for Haikou Taxi Dataset
...
# ============================================
# Custom Color Schemes (Avoiding Duplication)
# ============================================
...
class HaikouDatasetFigureGenerator:
    """Generate visualization figures for Haikou taxi dataset"""
    
    def __init__(self, data_dir, output_path='haikou_spatial_distribution.png'):
        self.data_dir = data_dir
        self.output_path = output_path
        self.df = None
        
        # Haikou core urban area boundaries (from processed data)
        # Longitude: [110.277127, 110.376339], Latitude: [19.978258, 20.060804]
        # Coverage: 10.42 km × 9.16 km
        self.core_bounds = {
            'lon_min': 110.277127,
            'lon_max': 110.376339,
            'lat_min': 19.978258,
    ...
    def load_data(self, sample_ratio=0.15, max_files=5):
        """Load and preprocess taxi data"""
        print("Loading Haikou taxi dataset...")
        ...
        for file in csv_files[:max_files]:
            try:
                df_temp = pd.read_csv(file, dtype={'order_id': str})
        ...
        # Filter to reasonable geographic range (Haikou area)
        df = df[
            (df['starting_wgs84_lng'] >= 110.0) & (df['starting_wgs84_lng'] <= 110.6) &
            (df['starting_wgs84_lat'] >= 19.8) & (df['starting_wgs84_lat'] <= 20.2)
        ]
    ...
    def create_figure(self):
        ...
        # Create 2x2 subplot layout (different arrangement from Shanghai)
        fig = plt.figure(figsize=(16, 14))
        gs = fig.add_gridspec(2, 2, hspace=0.25, wspace=0.25)
        
        # Different subplot order to avoid duplication
        axes = [
    ...
    def _plot_full_distribution(self, ax):
        ...
        ax.scatter(self.df['starting_wgs84_lng'], self.df['starting_wgs84_lat'], 
    ...
    def _plot_core_extraction(self, ax):
        ...
        # Filter to core area
        core_df = self.df[
            (self.df['starting_wgs84_lng'] >= self.core_bounds['lon_min']) &
            (self.df['starting_wgs84_lng'] <= self.core_bounds['lon_max']) &
            (self.df['starting_wgs84_lat'] >= self.core_bounds['lat_min']) &
            (self.df['starting_wgs84_lat'] <= self.core_bounds['lat_max'])
        ]
        
        # Plot with different color scheme
        ax.scatter(core_df['starting_wgs84_lng'], core_df['starting_wgs84_lat'],
        ...
        lon_span_km = (self.core_bounds['lon_max'] - self.core_bounds['lon_min']) * 105.0
    ...
    def _plot_density_heatmap(self, ax):
        ...
        for _, row in core_df.iterrows():
            lon_idx = np.searchsorted(lon_edges, row['starting_wgs84_lng']) - 1
            lat_idx = np.searchsorted(lat_edges, row['starting_wgs84_lat']) - 1
        ...
        # Plot heatmap with custom colormap (different from Shanghai)
...
def main():
    ...
    print("=" * 80)
    print("Haikou Taxi Dataset Visualization Generator")
    print("=" * 80)
    print()
    
    # Set paths
    data_dir = '/root/lanyun-tmp/data_code/raw_data/海口打车数据'
    output_path = '/root/lanyun-tmp/data_code/haikou_spatial_distribution.png'
    
    # Create generator
    generator = HaikouDatasetFigureGenerator(data_dir, output_path)
    
    try:
        # Load data (sample 15% for better visualization)
        generator.load_data(sample_ratio=0.15, max_files=5)
        ...
        print("  • Color scheme: Custom (avoiding duplication)")
