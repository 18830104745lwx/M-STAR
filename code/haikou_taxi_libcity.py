...
# -*- coding: utf-8 -*-
"""
海口打车数据转换为LibCity格式 - 完整流程
方案: 20×20网格，核心市区，对标TaxiBJ
...
class HaikouTaxiLibCityConverter:
    ...
    def step1_load_and_clean_data(self, max_files=None):
        ...
        for file in tqdm(files, desc="加载文件"):
            try:
                ...
                # 指定order_id为字符串类型，避免科学计数法导致精度丢失
        ...
        # 1. 删除空值（必须包含arrive_time，与成都逻辑一致）
        raw_data = raw_data.dropna(subset=['departure_time', 'arrive_time',
                                           'starting_wgs84_lng', 'starting_wgs84_lat', 
                                           'dest_wgs84_lng', 'dest_wgs84_lat'])
        print(f"  去空值后: {len(raw_data):,} 条")
        
        # 2. 时间格式转换（海口数据格式：2017/7/1 6:20）
        # 使用向量化操作，比apply快100倍以上
        print("  时间格式转换中...")
        ...
        # 删除时间转换失败的记录（与成都逻辑一致，必须有有效的开始和结束时间）
        raw_data = raw_data.dropna(subset=['departure_time', 'arrive_time'])
        print(f"  时间格式化后: {len(raw_data):,} 条")
        
        # 3. 删除重复订单（海口数据order_id列有精度问题，使用时空信息组合去重）
        # 在时间转换后去重，确保基于datetime类型而非字符串
        ...
        print(f"  去重: {before_dedup:,} → {after_dedup:,} 条 (删除 {before_dedup-after_dedup:,} 条重复)")
        
        # 4. 过滤时间异常（arrive_time > departure_time，与成都逻辑一致）
        raw_data = raw_data[raw_data['arrive_time'] > raw_data['departure_time']]
        print(f"  时间异常过滤后: {len(raw_data):,} 条")
    ...
    def step2_create_grid_system(self):
        ...
        # 估算每个网格的实际大小（海口约北纬20度）
        lon_per_grid = (lon_max - lon_min) / self.grid_cols
        lat_per_grid = (lat_max - lat_min) / self.grid_rows
        grid_width_km = lon_per_grid * 105.0  # 海口约北纬20度，1度经度约105km
        grid_height_km = lat_per_grid * 111.0  # 1度纬度约111km
    ...
    def generate_statistics_report(self):
        ...
        print(f"\n📏 网格信息")
        lon_span = (self.grid_info['lon_max'] - self.grid_info['lon_min']) * 105.0  # 海口纬度约20度
...
def main():
    ...
    try:
        ...
        print("2. 在LibCity配置文件中指定数据集名称: HaikouTaxi20x20")
        print("3. 运行您的交通预测模型")
