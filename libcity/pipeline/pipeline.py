...
def run_model(task=None, model_name=None, dataset_name=None, config_file=None,
    ...
    # 【修改】支持在验证集上评估（用于超参数选择）
    eval_on_validation = config.get('eval_on_validation', False)
    if eval_on_validation:
        logger.info('Evaluating on VALIDATION set (for hyperparameter tuning)')
        executor.evaluate(valid_data)
    else:
        logger.info('Evaluating on TEST set (standard evaluation)')
